...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  //    val filtered = this.filter(t => t.retweets > elem.retweets)
  //    if (filtered.isInstanceOf[Empty]) elem
  ...
  def descAcc(acc: TweetList, set: TweetSet): TweetList = {
    if (set.isInstanceOf[Empty]) acc
    else {
      val least = set.leastRetweeted
      val rest = set.remove(least)
      descAcc(new Cons(least, acc), rest)
    }
  }
}
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(t => google.exists(elem => t.text.contains(elem)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(t => apple.exists(elem => t.text.contains(elem)))
